...
// Before a new wait is initialized, every condition belongs to the instant editor. Keeping the
// boundary at the end prevents those stale rows from rendering as additional wait conditions.
const waitConditionCount = ref(conditions.value.length);
...
const additionalConditionIndexes = computed(() =>
  conditions.value
    .slice(waitConditionCount.value)
    .map((_, index) => waitConditionCount.value + index)
);
...
const initialWaitConditionCount = () => {
  const baseCount = selectedTrigger.value === 'customer_unresponsive' ? 2 : 1;
  return conditions.value[baseCount]?.attribute_key === 'inbox_id'
    ? baseCount + 1
    : baseCount;
};
...
const hydrateFromRule = () => {
  ...
  if (eventName.value === 'message_created') {
    ...
    waitConditionCount.value = initialWaitConditionCount();
    return;
  }
...
const applyTrigger = ({ preserveAdditional = true } = {}) => {
  const additionalConditions = preserveAdditional
    ? conditions.value.slice(waitConditionCount.value)
    : [];
  ...
  waitConditionCount.value = waitConditions.length;
  conditions.value = [
    ...waitConditions,
...
onMounted(() => {
  applyTrigger({ preserveAdditional: props.isSavedWait });
});
